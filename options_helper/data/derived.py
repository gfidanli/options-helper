...
class DerivedStore:
    ...
    def load(self, symbol: str) -> pd.DataFrame:
        ...
        if not path.exists():
            return pd.DataFrame(columns=DERIVED_COLUMNS_V1)
        try:
            df = pd.read_csv(path)
        ...
        for col in DERIVED_COLUMNS_V1:
            if col not in df.columns:
                df[col] = float("nan")
        df = df[DERIVED_COLUMNS_V1].copy()
    ...
    def upsert(self, symbol: str, row: DerivedRow) -> Path:
        ...
        data = {k: payload.get(k) for k in DERIVED_COLUMNS_V1}
        df = pd.concat([df, pd.DataFrame([data])], ignore_index=True)

        # Keep deterministic ordering.
        df = df.sort_values(["date"], ascending=True, na_position="last")
        df = df[DERIVED_COLUMNS_V1]
