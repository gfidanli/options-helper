...
class _CompareCoreDiffs(BaseModel):
    spot_change: float
    spot_change_pct: float | None
    pc_oi_from: float | None
    pc_oi_to: float | None
    pc_oi_change: float | None
    pc_vol_from: float | None
    pc_vol_to: float | None
    pc_vol_change: float | None
...
def compute_compare_report(
    *,
    ...
    warnings: list[str] = []
    selected_exp = _select_common_expiries(from_df=from_df, to_df=to_df, warnings=warnings)
    ...
    core = _compute_core_diffs(spot_from=spot_from, spot_to=spot_to, report_from=report_from, report_to=report_to)
    expiry_diffs = _compute_expiry_diffs(report_from=report_from, report_to=report_to)
    walls_overall, walls_by_expiry = _compute_walls_deltas(
        from_df=from_df,
        to_df=to_df,
    ...
    flow_top_contracts, flow_top_expiries = _compute_flow_deltas(
        from_df=from_df,
        to_df=to_df,
        spot_to=spot_to,
        top=top,
    ...
    report = CompareReport(
        ...
        spot_change=core.spot_change,
        spot_change_pct=core.spot_change_pct,
        pc_oi_ratio_from=core.pc_oi_from,
        pc_oi_ratio_to=core.pc_oi_to,
        pc_oi_ratio_change=core.pc_oi_change,
        pc_volume_ratio_from=core.pc_vol_from,
        pc_volume_ratio_to=core.pc_vol_to,
        pc_volume_ratio_change=core.pc_vol_change,
...
def _select_common_expiries(*, from_df: pd.DataFrame, to_df: pd.DataFrame, warnings: list[str]) -> list[date]:
    ...
    return select_expiries(common_exp, mode="near")


def _compute_core_diffs(
    *,
    spot_from: float,
    spot_to: float,
    report_from: ChainReport,
    report_to: ChainReport,
) -> _CompareCoreDiffs:
...
def _compute_expiry_diffs(*, report_from: ChainReport, report_to: ChainReport) -> list[ExpiryCompare]:
    ...
    for exp in report_to.included_expiries:
        ...
        if a is None or b is None:
            continue
        iv_change_pp = (b.atm_iv - a.atm_iv) * 100.0 if (a.atm_iv is not None and b.atm_iv is not None) else None
        em_change = b.expected_move - a.expected_move if (a.expected_move is not None and b.expected_move is not None) else None
        em_pct_change_pp = (
            (b.expected_move_pct - a.expected_move_pct) * 100.0
...
def _compute_walls_deltas(
    *,
    ...
    report_to: ChainReport,
    top: int,
) -> tuple[WallsDelta, list[WallsDeltaByExpiry]]:
    ...
    call_strikes = sorted({lvl.strike for lvl in report_from.walls_overall.calls}.union({lvl.strike for lvl in report_to.walls_overall.calls}))
    put_strikes = sorted({lvl.strike for lvl in report_from.walls_overall.puts}.union({lvl.strike for lvl in report_to.walls_overall.puts}))
    walls_overall = WallsDelta(
    ...
    for exp in report_to.included_expiries:
        ...
        walls_by_expiry.append(
            WallsDeltaByExpiry(
                ...
                walls=WallsDelta(
                    calls=_wall_deltas(
                        from_oi=_oi_by_strike(from_df, option_type="call", expiry=exp),
                        to_oi=_oi_by_strike(to_df, option_type="call", expiry=exp),
                        strikes=sorted({lvl.strike for lvl in fw.calls}.union({lvl.strike for lvl in tw.calls})),
                        top=top,
                    ),
                    puts=_wall_deltas(
                        from_oi=_oi_by_strike(from_df, option_type="put", expiry=exp),
                        to_oi=_oi_by_strike(to_df, option_type="put", expiry=exp),
                        strikes=sorted({lvl.strike for lvl in fw.puts}.union({lvl.strike for lvl in tw.puts})),
                        top=top,
                    ),
                ),
            )
        )
    return walls_overall, walls_by_expiry


def _compute_flow_deltas(
    *,
    from_df: pd.DataFrame,
    to_df: pd.DataFrame,
    spot_to: float,
    top: int,
    warnings: list[str],
) -> tuple[list[FlowContract], list[FlowExpiryNet]]:
    try:
        flow = compute_flow(to_df, from_df, spot=spot_to)
        _append_flow_warnings(flow=flow, warnings=warnings)
        non_zero_flow = _non_zero_notional_flow(flow)
        if flow.empty or "deltaOI_notional" not in flow.columns or non_zero_flow.empty:
            return [], []
        contracts = _build_flow_top_contracts(non_zero_flow=non_zero_flow, top=top)
        expiries = _build_flow_top_expiries(non_zero_flow=non_zero_flow, top=top)
...
def _append_flow_warnings(*, flow: pd.DataFrame, warnings: list[str]) -> None:
    if flow.empty:
        return
    ...
    if delta_oi is not None and prev_matched > 0 and not bool((delta_oi.abs() > 0).any()):
        warnings.append(f"oi_unchanged_between_snapshots ({prev_matched} contracts matched)")


def _non_zero_notional_flow(flow: pd.DataFrame) -> pd.DataFrame:
    if flow.empty or "deltaOI_notional" not in flow.columns:
        return pd.DataFrame()
    working = flow.copy()
    working["deltaOI_notional"] = pd.to_numeric(working.get("deltaOI_notional"), errors="coerce")
    return working[working["deltaOI_notional"].abs() > 0].copy()


def _build_flow_top_contracts(*, non_zero_flow: pd.DataFrame, top: int) -> list[FlowContract]:
    flow_top_contracts: list[FlowContract] = []
    ranked = non_zero_flow.assign(_abs=non_zero_flow["deltaOI_notional"].abs()).sort_values("_abs", ascending=False)
    for _, row in ranked.head(top).iterrows():
        strike = row.get("strike")
        strike_val = float(strike) if strike is not None and not pd.isna(strike) else None
        osi_raw = row.get("osi")
        osi_val = str(osi_raw).strip() or None if osi_raw is not None and not pd.isna(osi_raw) else None
        ...
        delta_notional_val = float(delta_notional) if delta_notional is not None and not pd.isna(delta_notional) else None
    ...
    return flow_top_contracts


def _build_flow_top_expiries(*, non_zero_flow: pd.DataFrame, top: int) -> list[FlowExpiryNet]:
