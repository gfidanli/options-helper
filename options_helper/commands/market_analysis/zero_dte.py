...
_ZERO_DTE_STUDY_SYMBOL_OPT = typer.Option(
    DEFAULT_PROXY_UNDERLYING,
    "--symbol",
    help="Proxy underlying symbol (default: SPY).",
)
_ZERO_DTE_STUDY_START_DATE_OPT = typer.Option(
    None,
    "--start-date",
    help="Start session date (YYYY-MM-DD). Defaults to 60 calendar days before end date.",
)
_ZERO_DTE_STUDY_END_DATE_OPT = typer.Option(
    None,
    "--end-date",
    help="End session date (YYYY-MM-DD). Defaults to latest available intraday session.",
)
_ZERO_DTE_STUDY_DECISION_MODE_OPT = typer.Option(
    DecisionMode.FIXED_TIME.value,
    "--decision-mode",
    help="Decision mode: fixed_time|rolling.",
)
_ZERO_DTE_STUDY_DECISION_TIMES_OPT = typer.Option(
    "10:30",
    "--decision-times",
    help="Comma-separated decision times in ET (HH:MM) for fixed_time mode.",
)
_ZERO_DTE_STUDY_ROLLING_INTERVAL_OPT = typer.Option(
    15,
    "--rolling-interval-minutes",
    min=1,
    help="Rolling decision interval minutes when --decision-mode rolling.",
)
_ZERO_DTE_STUDY_RISK_TIERS_OPT = typer.Option(
    "0.005,0.01,0.02,0.05",
    "--risk-tiers",
    help="Comma-separated breach-probability tiers (for example: 0.005,0.01,0.02,0.05).",
)
_ZERO_DTE_STUDY_STRIKE_GRID_OPT = typer.Option(
    "-0.03,-0.02,-0.015,-0.01,-0.005",
    "--strike-grid",
    help="Comma-separated strike returns from entry (negative values).",
)
_ZERO_DTE_STUDY_FILL_MODEL_OPT = typer.Option(
    FillModel.BID.value,
    "--fill-model",
    help="Entry fill model: bid|mid|ask.",
)
_ZERO_DTE_STUDY_ENTRY_SLIPPAGE_OPT = typer.Option(0.0, "--entry-slippage-bps", min=0.0, help="Entry slippage basis points.")
_ZERO_DTE_STUDY_EXIT_SLIPPAGE_OPT = typer.Option(0.0, "--exit-slippage-bps", min=0.0, help="Exit slippage basis points.")
_ZERO_DTE_STUDY_TRAIN_SESSIONS_OPT = typer.Option(20, "--train-sessions", min=1, help="Walk-forward train sessions.")
_ZERO_DTE_STUDY_TEST_SESSIONS_OPT = typer.Option(5, "--test-sessions", min=1, help="Walk-forward test sessions.")
_ZERO_DTE_STUDY_STEP_SESSIONS_OPT = typer.Option(5, "--step-sessions", min=1, help="Walk-forward step sessions.")
_ZERO_DTE_STUDY_MIN_TRAINING_ROWS_OPT = typer.Option(
    20,
    "--min-training-rows",
    min=1,
    help="Minimum in-sample rows required before a walk-forward fit.",
)
_ZERO_DTE_STUDY_TOP_K_OPT = typer.Option(3, "--top-k-per-tier", min=1, help="Recommended strike count per risk tier.")
_ZERO_DTE_STUDY_STRICT_PREFLIGHT_OPT = typer.Option(
    True,
    "--strict-preflight/--no-strict-preflight",
    help="Fail closed if data sufficiency preflight does not pass.",
)
_ZERO_DTE_STUDY_INTRADAY_DIR_OPT = typer.Option(Path("data/intraday"), "--intraday-dir", help="Intraday partition root.")
_ZERO_DTE_STUDY_SNAPSHOT_CACHE_DIR_OPT = typer.Option(
    Path("data/options_snapshots"),
    "--snapshot-cache-dir",
    help="Options snapshot cache root.",
)
_ZERO_DTE_STUDY_CANDLE_CACHE_DIR_OPT = typer.Option(Path("data/candles"), "--candle-cache-dir", help="Daily candle cache root.")
_ZERO_DTE_STUDY_FORMAT_OPT = typer.Option("console", "--format", help="Output format: console|json")
_ZERO_DTE_STUDY_OUT_OPT = typer.Option(
    Path("data/reports"),
    "--out",
...
def zero_dte_put_study(
    symbol: str = _ZERO_DTE_STUDY_SYMBOL_OPT,
    start_date: str | None = _ZERO_DTE_STUDY_START_DATE_OPT,
    end_date: str | None = _ZERO_DTE_STUDY_END_DATE_OPT,
    decision_mode: str = _ZERO_DTE_STUDY_DECISION_MODE_OPT,
    decision_times: str = _ZERO_DTE_STUDY_DECISION_TIMES_OPT,
    rolling_interval_minutes: int = _ZERO_DTE_STUDY_ROLLING_INTERVAL_OPT,
    risk_tiers: str = _ZERO_DTE_STUDY_RISK_TIERS_OPT,
    strike_grid: str = _ZERO_DTE_STUDY_STRIKE_GRID_OPT,
    fill_model: str = _ZERO_DTE_STUDY_FILL_MODEL_OPT,
    entry_slippage_bps: float = _ZERO_DTE_STUDY_ENTRY_SLIPPAGE_OPT,
    exit_slippage_bps: float = _ZERO_DTE_STUDY_EXIT_SLIPPAGE_OPT,
    train_sessions: int = _ZERO_DTE_STUDY_TRAIN_SESSIONS_OPT,
    test_sessions: int = _ZERO_DTE_STUDY_TEST_SESSIONS_OPT,
    step_sessions: int = _ZERO_DTE_STUDY_STEP_SESSIONS_OPT,
    min_training_rows: int = _ZERO_DTE_STUDY_MIN_TRAINING_ROWS_OPT,
    top_k_per_tier: int = _ZERO_DTE_STUDY_TOP_K_OPT,
    strict_preflight: bool = _ZERO_DTE_STUDY_STRICT_PREFLIGHT_OPT,
    intraday_dir: Path = _ZERO_DTE_STUDY_INTRADAY_DIR_OPT,
    snapshot_cache_dir: Path = _ZERO_DTE_STUDY_SNAPSHOT_CACHE_DIR_OPT,
    candle_cache_dir: Path = _ZERO_DTE_STUDY_CANDLE_CACHE_DIR_OPT,
    format: str = _ZERO_DTE_STUDY_FORMAT_OPT,
    out: Path = _ZERO_DTE_STUDY_OUT_OPT,
) -> None:
    ...
    try:
        result = _run_zero_dte_study_workflow(
            symbol=sym,
    ...
    _render_zero_dte_study_result(console, result=result, output_fmt=output_fmt)
    _persist_zero_dte_study_outputs(console, result=result, output_fmt=output_fmt, out=out, symbol=sym)


def _run_zero_dte_study_workflow(
    *,
    symbol: str,
    start_date: str | None,
    end_date: str | None,
...
def _render_zero_dte_study_result(console: Console, *, result: Any, output_fmt: str) -> None:
    if output_fmt == "json":
...
def _persist_zero_dte_study_outputs(
    console: Console,
    *,
    result: Any,
    output_fmt: str,
    ...
    saved_artifact = _save_zero_dte_study_artifact(out, result.artifact, symbol=symbol)
    saved_model = None
    if result.active_model is not None:
        saved_model = _save_zero_dte_active_model(out, symbol=symbol, payload=result.active_model)
    if output_fmt == "json":
        return
