...
CRON_CMD="${REPO_DIR}/scripts/cron_daily_options_snapshot.sh"

# Default: weekdays at 16:00 America/Chicago time (market close + 60m).
CRON_TZ="America/Chicago"
SCHEDULE="0 16 * * 1-5"

BEGIN_MARK="# options-helper: daily options snapshot"
END_MARK="# end options-helper: daily options snapshot"
...
if [[ "${INSTALL}" -eq 0 ]]; then
  ...
  echo "${END_MARK}"
  echo
  echo "Logs: ${REPO_DIR}/data/logs/YYYY-MM-DD/options_snapshot.log"
  exit 0
fi
...
print(f"Logs: {repo_dir}/data/logs/YYYY-MM-DD/options_snapshot.log")
PY
