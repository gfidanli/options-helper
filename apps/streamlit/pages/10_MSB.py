...
from apps.streamlit.components.msb_page import (
    list_msb_symbols,
    load_msb_payload,
    normalize_symbol,
)
...
def _render_events_table(df: pd.DataFrame, *, title: str, tail_low_pct: float, tail_high_pct: float) -> None:
    ...
    display = display.rename(
        columns={
            "event_ts": "date",
            "broken_swing_ts": "broken_swing_date",
    ...
    for column in ("close", "break", "ext_pct", "extension_atr", "rsi", "ret_1d_pct", "ret_5d_pct", "ret_10d_pct"):
...
st.title("Market Structure Break Research")
st.caption("Informational and educational use only. Not financial advice.")
st.info("Read-only market structure break (MSB) research view from persisted daily candles.")
...
with st.sidebar:
    ...
    with st.expander("Advanced Settings", expanded=False):
        ...
        min_swing_distance = st.slider("Min bars from broken swing", min_value=1, max_value=10, value=1, step=1)
...
symbols, symbols_note = list_msb_symbols(database_path=database_arg)
if symbols_note:
...
payload, payload_note = load_msb_payload(
...
if payload_note:
    st.warning(f"MSB payload unavailable: {payload_note}")
    st.stop()
if payload is None:
    st.info("No MSB payload available.")
    st.stop()
...
metrics[1].metric("Daily MSB Events", str(int(counts.get("daily_events", 0))))
metrics[2].metric("Weekly MSB Events", str(int(counts.get("weekly_events", 0))))
metrics[3].metric(
...
_render_events_table(
    daily_events_df,
    title="Daily Market Structure Break Events",
    tail_low_pct=tail_low_pct,
    tail_high_pct=tail_high_pct,
)
_render_events_table(
    weekly_events_df,
    title="Weekly Market Structure Break Events (Week-Start Labels)",
    tail_low_pct=tail_low_pct,
    tail_high_pct=tail_high_pct,
)
