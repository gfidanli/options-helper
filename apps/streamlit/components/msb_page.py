...
def list_msb_symbols(*, database_path: str | Path | None = None) -> tuple[list[str], str | None]:
...
def _build_msb_payload_cached(
    *,
    ...
    weekly_signals = compute_msb_signals(
        weekly_ohlc,
        swing_left_bars=int(swing_left_bars),
        swing_right_bars=int(swing_right_bars),
        min_swing_distance_bars=int(min_swing_distance_bars),
...
def load_msb_payload(
    *,
    ...
    try:
        payload = _build_msb_payload_cached(
            symbol=normalize_symbol(symbol),
    ...
    *,
    ...
    for ev in events:
        ...
        formatted: dict[str, Any] = {
            ...
            "broken_swing_ts": display_swing_ts.date().isoformat(),
...
def _build_summary_rows(*, daily_events: list[dict[str, Any]], weekly_events: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    rows = []
    if daily_df.empty:
        rows.append(_summary_row("Daily Bullish MSB", daily_df))
        rows.append(_summary_row("Daily Bearish MSB", daily_df))
        rows.append(_summary_row("Daily MSB at RSI Extremes (Bullish)", daily_df))
        rows.append(_summary_row("Daily MSB at RSI Extremes (Bearish)", daily_df))
    else:
        bullish_daily = daily_df[daily_df["direction"] == "bullish"]
        bearish_daily = daily_df[daily_df["direction"] == "bearish"]
        rows.append(_summary_row("Daily Bullish MSB", bullish_daily))
        rows.append(_summary_row("Daily Bearish MSB", bearish_daily))
        rows.append(
            _summary_row(
                "Daily MSB at RSI Extremes (Bullish)",
                bullish_daily[bullish_daily["rsi_regime"].isin(["overbought", "oversold"])],
            )
        )
        rows.append(
            _summary_row(
                "Daily MSB at RSI Extremes (Bearish)",
                bearish_daily[bearish_daily["rsi_regime"].isin(["overbought", "oversold"])],
            )
        )

    if weekly_df.empty:
        rows.append(_summary_row("Weekly MSB + Daily Extension Extreme in Week (Bullish)", weekly_df))
        rows.append(_summary_row("Weekly MSB + Daily Extension Extreme in Week (Bearish)", weekly_df))
    else:
        weekly_extreme = weekly_df[weekly_df["week_has_daily_extension_extreme"] == True]  # noqa: E712
        rows.append(
            _summary_row(
                "Weekly MSB + Daily Extension Extreme in Week (Bullish)",
                weekly_extreme[weekly_extreme["direction"] == "bullish"],
            )
        )
        rows.append(
            _summary_row(
                "Weekly MSB + Daily Extension Extreme in Week (Bearish)",
